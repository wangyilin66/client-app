<template>
	<view class="top-popup-box">
		...
		<view class="popup-option" :style="{top:popupTop + 'rpx'}">
			<view class="option" v-for='(items,index) in options' :key="index" v-if="tabActive == index">
				<view class="poplist" v-for="(item,idx) in items" :key="idx" :class="{'ac':item.check}" @click="optclick(idx,index)">
					{{item.name}}
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			optclick(idx,index){
				...
				this.options[index][idx].check = true;
				console.log(this.options[index][idx].name)
			}
		},
		created(){
